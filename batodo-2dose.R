...
batodo_2dose <- function(ntrial, peff, ptox, n1, nsample, theta0, theta0T, weightu, delta1, cutoff_utility){
  ...
  nsampling <- 10000
  nburnin <- 1000
  cutoff_interim <- cutoff_interimT <- 0.231
  cutoff_final <- cutoff_finalT <- 0.8378
  ...
  fa.postE <- post_model_faE <- fa_pE_bma <- re_sampleE <- matrix(0,nrow=nks,ncol=nds)
  fa.postT <- post_model_faT <- fa_pT_bma <- re_sampleT <- matrix(0,nrow=nks,ncol=nds)
  ...
  bma_ndlm_marginl <- function(datar, datan, nks, nds, nsampling,model,nmodel, theta0,zeta.mu,zeta.tau,tau.theta1){
    ...
    rmu11 <- rmu21 <- matrix(0,nsampling,nmodel) ## randomly generated mu for tumor type 1
    rmu12 <- rmu22 <- matrix(0,nsampling,nmodel) ## randomly generated mu for tumor type 2
    rmu13 <- rmu23 <- matrix(0,nsampling,nmodel) ## randomly generated mu for tumor type 3
    rmu14 <- rmu24 <- matrix(0,nsampling,nmodel) ## randomly generated mu for tumor type 4
  ...
  for(itrial in 1:ntrial){
    ...
    for(ik in 1:nks){
      for(id in 1:nds){
        if(ia.postE[ik,id]<cutoff_interim | ia.postT[ik,id]<cutoff_interimT){ ## stop the dose arm if overly toxic or futile
    ...
    if(sum(data.FA==nsample)>0){
      ...
      post_model_faE <- result_faE$post_model # posterior mean estimate of efficacy
      fa_pE_bma <- result_faE$post_p_bma # posterior model weight
      ...
      post_model_faT <- result_faT$post_model # posterior mean estimate of toxicity
      fa_pT_bma <- result_faT$post_p_bma # posterior model weight
